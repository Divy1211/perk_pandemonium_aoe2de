...
for NUMBER_OF_PLAYERS in range(1, 9):
    inname = f"Parser Bases/Base {NUMBER_OF_PLAYERS}p.aoe2scenario"
    ...
    # TODO: fix trigger objectives display orders
    # TODO: move information unrelated to perk p in the help file
    # TODO: make all short objectives into long objectives
    ...
    PAGES = [[]]
    CURRENT_PAGE = 0
    TRIGGER_UNITS = sorted([(hero.ID, hero.ICON_ID, hero.DEAD_ID, False) for hero in HeroInfo],
                           key=lambda x: HeroInfo(x).name)
    ...
    GARRISONABLE_BY_ALL = [BuildingInfo.TOWN_CENTER.ID, BuildingInfo.WATCH_TOWER.ID, BuildingInfo.GUARD_TOWER.ID,
                           ...
                           BuildingInfo.DONJON.ID, BuildingInfo.KREPOST.ID, BuildingInfo.CASTLE.ID,
                           UnitInfo.BATTERING_RAM.ID,
                           UnitInfo.CAPPED_RAM.ID, UnitInfo.SIEGE_RAM.ID, UnitInfo.SIEGE_TOWER.ID,
